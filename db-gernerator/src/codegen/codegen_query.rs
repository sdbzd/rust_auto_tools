...
use super::{CodeGenerator, RustFileImpl, RustStruct, RustStructField};
...
pub fn parse_query_as_func(
                    ...
                    Some(ct) => {
                        ...
                        if cts.len() == cns.len() {
                            ...
                            while ii < cts.len() {
                                let fdname = cns[ii].clone();
                                testexpr
                                    .push_str(format!(" param.{}.is_some() &&", fdname).as_str());
                                ii += 1;
                            }
    ...
    if paging {
        body.push(
            "return rb.fetch_page(&sql, rb_args, &PageRequest::new(curr, size)).await".to_string(),
        );
    } else {
        if onerow {
