use std::{collections::btree_map::VacantEntry, fs::File, vec};
...
pub fn parse_yaml_as_file(conf_path: &str, filename: &String) -> RustFileImpl {
    let mut sts = parse_yaml_as_struct(conf_path);
        ...
        Some(hs) => {
            for ks in hs.clone() {
                ...
                let value_type = match ks.1.clone() {
                    Yaml::Real(s) => "f64".to_string(),
                    Yaml::Integer(it) => "i64".to_string(),
                    Yaml::String(s) => "String".to_string(),
                    Yaml::Boolean(s) => "bool".to_string(),
                    Yaml::Array(tt) => {
                        let fnd = &tt[0];
                ...
                st.fields.push(RustStructField {
                    is_pub: true,
                    column_name: key.clone().to_string(),
