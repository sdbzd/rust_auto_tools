use crate::config::{get_redis_manager, AppConfig};
...
pub fn f32_to_decimal(f: f32) -> Option<rbdc::decimal::Decimal> {
    match <rbdc::types::decimal::Decimal as std::str::FromStr>::from_str(format!("{:.2}", f).as_str()) {
        Ok(r) => Some(r),
        Err(_) => None,
    }
}

pub fn decimal_to_f32(dc: Option<rbdc::decimal::Decimal>) -> f32 {
    match dc {
...
pub fn make_decimal_negative(dc: Option<rbdc::decimal::Decimal>) -> Option<rbdc::decimal::Decimal> {
    match dc {
        Some(r) => match r.to_string().parse::<f32>() {
...
pub async fn redis_get_string(key: &str) -> redis::RedisResult<String> {
    ...
    redis::cmd("GET")
        .arg(key)
        .query_async::<Connection, String>(&mut rcnn)
        .await
}
